...
export default class QasmCircuitPreviewView {
  ...
  requestUpdate(QASM, hard=false) {
    ...
    // var child = spawn('python3', [QASM.drawer_path, editor.buffer.file.path, QASM.reverse_bits]);
    //
  ...
  subscribeHandlers(QASM) {
    ...
    QASM.subscriptions.add(atom.workspace.onDidOpen((activeItem) => {

      console.log("YO>", activeItem)

      editor = activeItem.pane.activeItem;
